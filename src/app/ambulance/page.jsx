...
const AMBULANCE_TYPES = [
  {
    ...
    bg: "#eff6ff",
    border: "#bfdbfe",
    features: ["Trained EMT on-board", "Oxygen Support", "First Aid Kit", "Stretcher"],
  },
  {
    id: "als",
    icon: "🏥",
    ...
    bg: "#fff1f2",
    border: "#fecdd3",
    features: ["Paramedic + Doctor", "Cardiac Monitor", "Defibrillator", "Ventilator"],
    badge: "🔥 Most Booked",
  },
];
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div
          ...
          style={{ background: "linear-gradient(135deg, #0f172a 0%, #7f1d1d 60%, #dc2626 100%)" }}
        >
          {/* Decorative circles */}
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 md:py-16 relative z-10">
            {/* Breadcrumb */}
            <div className="flex items-center gap-2 text-xs mb-6" style={{ color: "rgba(252,165,165,0.8)" }}>
            ...
            <div className="flex flex-col md:flex-row items-center gap-10">
              <div className="flex-1">
                {/* Siren badge */}
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-white text-xs font-bold mb-5"
                ...
                <p className="text-slate-300 text-sm md:text-base leading-relaxed max-w-lg mb-8">
                  Fastest emergency medical transport in your city. BLS, ALS &amp; Air Ambulance available. 
                  Dispatched within minutes — because every second counts.
                </p>
                ...
                <div className="flex flex-wrap gap-6">
                  {[
                    ...
                    <div key={s.label}>
                      ...
                      <p className="text-xs font-medium" style={{ color: "rgba(252,165,165,0.8)" }}>{s.label}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="w-full md:w-auto flex-shrink-0">
                <div className="bg-white rounded-3xl p-6 shadow-2xl text-center" style={{ minWidth: 240 }}>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Emergency Helpline</p>
                  <a
                    href="tel:108"
                  ...
                  <p className="text-[11px] text-slate-400 mb-4">Govt. Free Ambulance Service</p>
                  <div className="border-t border-slate-100 pt-4">
                    <p className="text-xs font-bold text-slate-500 mb-2">Private Booking</p>
                    <a
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <p className="text-center text-xs font-bold text-red-500 uppercase tracking-widest mb-2">Simple Process</p>
          <h2 className="text-2xl font-black text-slate-900 text-center mb-10">How It Works</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {STEPS.map((step, i) => (
              <div key={i} className="relative amb-step-line amb-step amb-slide flex flex-col items-center text-center"
                style={{ animationDelay: `${i * 0.1}s` }}>
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl mb-3 shadow-md"
                  style={{ background: i === 0 ? "#fee2e2" : i === 1 ? "#fff7ed" : i === 2 ? "#f0fdf4" : "#fef3c7" }}>
                  {step.icon}
                </div>
                <p className="font-bold text-sm text-slate-900 mb-1">{step.label}</p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-16">
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="amb-slide" style={{ animationDelay: "0.1s" }}>
              <h2 className="text-lg font-black text-slate-900 mb-1">Choose Ambulance Type</h2>
              <p className="text-xs text-slate-500 mb-5">Select the level of care needed</p>
                ...
                {AMBULANCE_TYPES.map((type) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: selected === type.id ? `0 0 0 3px ${type.color}22` : "none",
                    }}
                  >
                    {type.badge && (
                      <span className="absolute top-3 right-3 text-[10px] font-black text-white px-2 py-0.5 rounded-full"
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <p className="font-black text-sm text-slate-900">{type.title}</p>
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                            style={{ background: type.bg, color: type.color, border: `1px solid ${type.border}` }}>
                            {type.subtitle}
                          </span>
                        </div>
                        <p className="text-[12px] text-slate-500 mb-3">{type.desc}</p>
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {type.features.map((f) => (
                            <span key={f} className="text-[10px] font-semibold px-2 py-1 rounded-lg"
                              style={{ background: selected === type.id ? `${type.color}18` : "#f1f5f9", color: selected === type.id ? type.color : "#64748b" }}>
                              ✓ {f}
                            </span>
                          ))}
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-black" style={{ color: type.color }}>
                            {type.price} <span className="text-[10px] font-normal text-slate-400">onwards</span>
                          </span>
                          <span className="text-[11px] text-slate-400">⏱ ETA: {type.eta}</span>
                        </div>
                      </div>
                      <div className="flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center mt-1"
                        style={{ borderColor: selected === type.id ? type.color : "#cbd5e1" }}>
                        {selected === type.id && (
            ...
            <div className="amb-slide" style={{ animationDelay: "0.2s" }}>
              <h2 className="text-lg font-black text-slate-900 mb-1">Booking Details</h2>
              <p className="text-xs text-slate-500 mb-5">Fill the form — we'll dispatch immediately</p>
              ...
              {submitted ? (
                <div className="bg-white rounded-3xl border border-green-200 p-10 flex flex-col items-center text-center shadow-lg amb-success">
                  ...
                  <p className="text-xs text-slate-400 mb-6">
                    ETA: <strong className="text-slate-700">{selectedType?.eta}</strong> · Our team will call you shortly
                  </p>
                  <div className="flex flex-col gap-3 w-full max-w-xs">
                    ...
                    <button onClick={() => { setSubmitted(false); setForm({ name:"",phone:"",pickup:"",drop:"",emergency:"",notes:"" }); }}
                ...
                <form onSubmit={handleSubmit}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1.5">
                        Patient / Caller Name <span className="text-red-500">*</span>
                      </label>
                  ...
                  <div>
                    ...
                    <input name="pickup" required value={form.pickup} onChange={handleChange}
                  ...
                  <div>
                    ...
                    <select name="emergency" value={form.emergency} onChange={handleChange}
                  ...
                  <div className="flex items-center gap-3 rounded-xl p-3.5 border"
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-black text-slate-900">{selectedType?.title}</p>
                      <p className="text-[10px] text-slate-500">ETA: {selectedType?.eta} · From {selectedType?.price}</p>
                    </div>
                    <button type="button" onClick={() => document.querySelector('[data-scroll="types"]')?.scrollIntoView({ behavior: "smooth" })}
                      className="text-[10px] font-bold underline" style={{ color: selectedType?.color }}>
                      Change
                    </button>
                  </div>
                  ...
                  <p className="text-[10px] text-center text-slate-400">
                    For life-threatening emergencies, call <a href="tel:108" className="font-bold text-red-500">108</a> (free govt. ambulance) immediately.
                  </p>
                </form>
              )}
            </div>
